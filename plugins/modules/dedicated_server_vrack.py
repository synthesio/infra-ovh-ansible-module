...
def run_module():
    ...
    if module.check_mode:
        module.exit_json(
            msg=f"{service_name} succesfully {state} on {vrack} - (dry run mode)",
            changed=True,
        )
    ...
    result = client.get(
        f"/dedicated/server/{service_name}/virtualNetworkInterface", mode="vrack"
    )
    ...
    if len(result):
        ...
        for new_server in is_already_registered:
            if new_server["dedicatedServer"] == service_name:
                if state == "present":
                    module.exit_json(
                        msg=f"{service_name} is already registered on new {vrack}",
                        changed=False,
                    )
                if state == "absent":
                    ...
                    module.exit_json(
                        msg=f"{service_name} has been deleted from new {vrack}",
                        changed=True,
                    )

        if state == "absent":
            module.exit_json(
                msg=f"{service_name} is not present on new {vrack}, don't remove it",
                changed=False,
            )
        ...
        module.exit_json(
            msg=f"{service_name} has been added to new {vrack}",
            changed=True,
        )

    # Old generation
    else:
        is_already_registered = client.get(f"/vrack/{vrack}/dedicatedServer")

        for old_server in is_already_registered:
            if old_server == service_name:
                if state == "present":
                    module.exit_json(
                        msg=f"{service_name} is already registered on old {vrack}",
                        changed=False,
                    )
                if state == "absent":
                    result = client.delete(
                        f"/vrack/{vrack}/dedicatedServer/{service_name}%s"
                    )
                    module.exit_json(
                        msg=f"{service_name} has been deleted from old {vrack}",
                        changed=True,
                    )

        if state == "absent":
            module.exit_json(
                msg=f"{service_name} is not present on old {vrack}, don't remove it",
                changed=False,
            )

        # Server is not yet registered on vrack, go for it
        client.post(f"/vrack/{vrack}/dedicatedServer", dedicatedServer=service_name)
        module.exit_json(
            msg=f"{service_name} has been added to old {vrack}",
            changed=True,
        )
