...
def run_module():
    ...
    if module.check_mode:
        module.exit_json(msg=f"template {template} is now {state} - dry run mode")

    template_list = client.get("/me/installationTemplate")

    if state == "absent":
        if src_template in template_list:
            client.delete(f"/me/installationTemplate/{src_template}")

            module.exit_json(
                msg=f"Template {src_template} succesfully deleted",
                changed=True,
            )
        else:
            module.exit_json(
                msg=f"Template {src_template} does not exists",
                changed=False,
            )
    ...
    # state == 'present'
    if src_template in template_list:
        module.exit_json(msg=f"Template {src_template} already exists", changed=False)
    ...
    client.put(f"/me/installationTemplate/{conf['templateName']}", **templates)
    ...
    if conf["isHardwareRaid"]:
        result = client.get(
            f"/dedicated/server/{service_name}/install/hardwareRaidProfile"
        )
    ...
    for k in conf["partition"]:
        ...
        if "raid" in partition.keys():
            client.post(
                f"/me/installationTemplate/{conf['templateName']}/partitionScheme/{conf['partitionScheme']}%s/partition",
    ...
    client.post(f"/me/installationTemplate/{conf['templateName']}/checkIntegrity")
    module.exit_json(
