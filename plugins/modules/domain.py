...
options:
    ...
    record_type:
        required: false
        description: The DNS record type, any of the type supported by OVH API
        default: A
    state:
        required: false
...
def run_module():
    ...
    if module.check_mode:
        module.exit_json(
            msg="{} set to {}.{} ! - (dry run mode)".format(value, name, domain)
        )
    ...
    if state == "present":
        ...
        if existing_records:
            ...
            if len(existing_records) > 1:
                module.fail_json(
                    msg="More than one record match the name {} in domain {}, this module won't update all of these records.".format(
                        name, domain
                    )
                )
            ...
            try:
                ...
                module.exit_json(
                    msg="{} record has been updated: {} is now targeting {}.{}".format(
                        record_type, value, name, domain
                    ),
                    changed=True,
                )
            except APIError as api_error:
        ...
        try:
            ...
            client.post("/domain/zone/%s/refresh" % domain)
            module.exit_json(
                msg="{} is now targeting {}.{}".format(value, name, domain),
                changed=True,
            )
        except APIError as api_error:
            module.fail_json(msg="Failed to call OVH API: {0}".format(api_error))
